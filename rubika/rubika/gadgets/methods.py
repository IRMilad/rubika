...
![Rubika Icon](https://github.com/IRMilad/rubika/raw/main/icon.png)
...
Returns:
    dict: the output can be given to __call__ client and run it


* ## tips
# ignore the (Module 'rubika.gadgets.methods' has no '*' member) error
"""
...
class Functions:
    ...
    def to_metadata(cls, value, *args, **kwargs):
        pattrn: str = r'`(.*)`|\*\*(.*)\*\*|__(.*)__|\[(.*)\]\((\S+)\)'
        conflict: int = 0
        meta_data_parts = []
        for markdown in re.finditer(pattrn, value):
            span = markdown.span()
            if markdown.group(0).startswith('`'):
                value = re.sub(pattrn, r'\1', value, count=1)
                ...
                value = re.sub(pattrn, r'\2', value, count=1)
                meta_data_parts.append(
                    {
                        'type': 'Bold',
                        'from_index': span[0] - conflict,
                ...
                value = re.sub(pattrn, r'\3', value, count=1)
                meta_data_parts.append(
                    {
                        'type': 'Italic',
                        'from_index': span[0] - conflict,
                ...
                value = re.sub(pattrn, r'\4', value, count=1)
